  ...
  try {
    ...
    if (forms.length === 0) {
      sendStatusUpdate(
        ...
        'No forms were found on this page. Please navigate to a page with a form.'
      );
      return;
    }
...
const fillFormWithData = (data: Record<string, string>) => {
  try {
    ...
    // Find all form elements again to ensure we have the latest state
    const forms = document.querySelectorAll('form');
    if (forms.length === 0) {
      sendStatusUpdate(
        'error',
        'Form no longer available on this page.',
    ...
    capturedFormData.fields.forEach((field) => {
      ...
      if (!element && field.name) {
        element = form.querySelector(`[name="${field.name}"]`);
      }
